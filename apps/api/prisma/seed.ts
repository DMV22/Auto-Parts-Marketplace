...
import { runInTransactionBatches } from '../src/prisma/transaction-batches';
import {
  demoActivityLogs,
  demoBrands,
  demoCartItems,
...
const SEED_TRANSACTION_BATCH_SIZE = 50;
  ...
  try {
    ...
    const seedInBatches = <TItem>(
      ...
      runInTransactionBatches(
        items,
        SEED_TRANSACTION_BATCH_SIZE,
        (operation) => prisma.$transaction(operation, SEED_TRANSACTION_OPTIONS),
        writeItem,
      );

    await seedInBatches(demoUsers, async (transaction, user) => {
    ...
    await seedInBatches(demoCustomerProfiles, async (transaction, profile) => {
    ...
    await seedInBatches(demoSuppliers, async (transaction, supplier) => {
    ...
    await seedInBatches(demoSupplierUsers, async (transaction, membership) => {
    ...
    await seedInBatches(demoCategories, async (transaction, category) => {
    ...
    await seedInBatches(demoBrands, async (transaction, brand) => {
    ...
    await seedInBatches(demoProducts, async (transaction, product) => {
    ...
    await seedInBatches(demoProductVariants, async (transaction, variant) => {
    ...
    await seedInBatches(demoVehicleMakes, async (transaction, make) => {
    ...
    await seedInBatches(demoVehicleModels, async (transaction, model) => {
    ...
    await seedInBatches(demoEngineTypes, async (transaction, engine) => {
      ...
      engineIds.set(`${engine.generationKey}:${engine.code}`, seededEngine.id);
    });

    await seedInBatches(demoFitmentRules, async (transaction, fitment) => {
      await transaction.fitmentRule.upsert({
        where: { id: fitment.id },
        update: {
          productVariantId: requireId(variantIds, fitment.variantSku),
          vehicleGenerationId: requireId(generationIds, fitment.generationKey),
    ...
    await seedInBatches(
      demoSavedVehicles,
      async (transaction, savedVehicle) => {
    ...
    await seedInBatches(
      demoSavedVehicles.filter((vehicle) => vehicle.isActive),
      async (transaction, savedVehicle) => {
    ...
    await seedInBatches(demoListings, async (transaction, listing) => {
    ...
    await seedInBatches(demoCarts, async (transaction, cart) => {
    ...
    await seedInBatches(demoCartItems, async (transaction, item) => {
    ...
    await seedInBatches(demoOrders, async (transaction, order) => {
    ...
    await seedInBatches(demoOrderItems, async (transaction, item) => {
    ...
    await seedInBatches(demoPaymentEvents, async (transaction, event) => {
    ...
    await seedInBatches(demoOrderStatusEvents, async (transaction, event) => {
    ...
    await seedInBatches(demoReturnRequests, async (transaction, request) => {
    ...
    await seedInBatches(demoNotes, async (transaction, note) => {
    ...
    await seedInBatches(demoActivityLogs, async (transaction, activity) => {
